...
def process_webpage(num_page, timestamp, url, canonical_url, page_contents, links_already_dispatched):
    ...
    page_links = extract_all_href_links(page_contents, canonical_url)
    follow_links = decide_which_links_to_follow(url_matching_pattern, terminal_extensions, canonical_url, url,
                                                page_links)
...
def consumer(filestream, url_matching_pattern, max_num_page_visited, \
    ...
    while (len(links_to_visit) > 0) and \
        ...
        links_to_follow = process_webpage(num_page, timestamp, url, canonical_url, page_contents,
...
if __name__ == "__main__":
    ...
    # Write a header to errors.txt
    with open('errors.txt', 'w+') as error_file:
        error_file.write(f'{" ".join(sys.argv)} started at {datetime.now()}\n')
